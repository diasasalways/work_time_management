...
const [zadaniedate,setdate]=useState('');
const [zadanieopis,setopis]=useState('');
...
function makemodal(date, opisanie){
  setModalVisible(true);
  setdate(date);
  setopis(opisanie);
  console.log('DATEXDD'+date);
  console.log(opisanie);
}

useEffect(() => {
  navigation.navigate('EmptyTask')
}, [])
  firebase.database()
  ...
  .once('value', websnapshot => {
    ...
    return (
        <KeyboardAvoidingView style={styles.container}>
         <TouchableOpacity style={{position: 'absolute', left:'7%',top:'0%'}}   onPress={ () => navigation.openDrawer()}> 
         <ImageBackground
    source={require('./menu_icon.png')}  style={{ width: 40,
      height: 40, paddingLeft:0}}/>  
        ...
        <View style={styles.container}>
         ...
         {returnArr.map((returnArr) => {
          return  (<View>
            <TouchableOpacity style={{flexDirection: 'column'}} onPress={() => {makemodal(returnArr.date, returnArr.zadanie)}}>
      ...
      alignItems: 'center', borderTopLeftRadius:22, borderTopRightRadius:22}}>
          ...
          <Text style={{fontSize:20,paddingTop:'10%', fontWeight: 'bold'}}>Задание </Text>
          <Text style={{left:'-28%',fontSize:14,paddingTop:'5%', fontWeight: 'bold'}}>Описание: {zadanieopis}</Text>
          <Text style={{left:'-20%',fontSize:14,paddingTop:'5%', fontWeight: 'bold'}}>Срок выполнения: {zadaniedate}</Text>
